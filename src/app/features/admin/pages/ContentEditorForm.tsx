...
type ContentFormData = Omit<
	MediaItem,
	'id' | 'totalViews' | 'lastMonthEarnings' | 'lifetimeEarnings'
> & {
...
interface ContentEditorFormProps {
	...
	onSubmit: (formData: Omit<MediaItem, 'id'>) => void
	onCancel: () => void
}
...
const ContentEditorForm = ({
	...
	useEffect(() => {
		if (isEditMode && initialData) {
			setFormData({
				...
				status:
					'status' in initialData ? (initialData as any).status : 'published'
			})
		}
	}, [initialData, isEditMode])

	useEffect(() => {
		const poster = formData.posterUrl
		const banner = formData.bannerUrl
	...
	}, [formData.posterUrl, formData.bannerUrl])
				...
				<form onSubmit={handleSubmit} className='space-y-10'>
					...
					{currentStep === 4 && (
						<div className='animate-fade-in space-y-6'>
							...
							{tagInputFields.map(
								({ field, label, value, setValue, placeholder }) => (
									<div key={field} className='space-y-2'>
										...
										<div className='flex'>
											...
											<button
												type='button'
												onClick={() => handleAddItem(field, value, setValue)}
